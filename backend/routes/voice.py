"""
voice.py — Translation + TTS routes
"""
...
def voice():
    try:
        ...
        print(f"[Voice] user={username} → lang={target_lang} text='{text[:60]}'")

        # 1. Translate (auto-detects input language)
        try:
        ...
        if not translated or not translated.strip():
            return jsonify({"success": False, "error": "Translation returned empty."}), 500
        ...
        try:
            ...
            db.execute(
                "INSERT INTO conversations(user_id,username,input_text,output_text,language,audio_url,created_at)"
                " VALUES(?,?,?,?,?,?,?)",
            ...
            print(f"[Voice] DB error (non-fatal): {e}")

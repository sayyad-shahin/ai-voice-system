...
@voice_routes.route("/voice", methods=["POST", "OPTIONS"])
def voice():
    ...
    try:
        ...
        if not text:
            return _cors({"success": False, "error": "No text received."}, 400)
        ...
        try:
            ...
            return _cors({"success": False, "error": "Translation failed."}, 500)

        if not translated or not translated.strip():
            return _cors({"success": False, "error": "Translation returned empty."}, 500)
        ...
        try:
            ...
            return _cors({"success": False, "error": "Voice generation failed."}, 500)

        if not audio_url:
            return _cors({"success": False,
                          "error": "Audio generation failed. Check ElevenLabs API key."}, 500)
        ...
        return _cors({"success": True, "text": final_text,
                      "audio": audio_url, "lang_name": lang_name})

    except Exception as e:
        print(f"[Voice] Unhandled: {e}")
        return _cors({"success": False, "error": "Processing failed."}, 500)


@voice_routes.route("/voices", methods=["GET", "OPTIONS"])
def voices():
    if request.method == "OPTIONS":
        return _options()
    try:
        return _cors({"success": True, "voices": get_voices()})
    except Exception as e:
        return _cors({"success": False, "voices": []})
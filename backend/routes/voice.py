    ...
    try:
        ...
        if len(text) > 15000:
            return jsonify({"success": False,
                            "error": "Input too long (max 15,000 chars)."}), 400
        ...
        if not translated or not translated.strip():
            return jsonify({"success": False,
                            "error": "Translation returned empty result."}), 500

        # 2. Post-process
        final_text = improve(translated)
        print(f"[Voice] Translated: {final_text[:100]}")

        # 3. TTS — passes target_lang so Google TTS fallback uses correct language
        try:
        ...
        if not audio_url:
            return jsonify({
                "success": False,
                "error": "Audio generation failed. Please check your ElevenLabs API key."
            }), 500
        ...
        print(f"[Voice] Unhandled error: {e}")
        return jsonify({"success": False,
